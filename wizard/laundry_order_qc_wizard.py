...
class LaundryOrderQCWizard(models.TransientModel):
    ...
    # Checklist
    clean_check = fields.Boolean(string='Bersih / Tidak ada Noda', default=True)
    dry_check = fields.Boolean(string='Kering Sempurna', default=True)
    iron_check = fields.Boolean(string='Setrika Rapi', default=True)
    perfume_check = fields.Boolean(string='Sudah Diberi Parfum', default=True)
    qty_check = fields.Boolean(string='Jumlah Sesuai (Pcs/Kg)', default=True)
    ...
    def action_confirm_qc(self):
        ...
        if not all([self.clean_check, self.dry_check, self.iron_check, self.perfume_check, self.qty_check]):
        ...
        qc_vals = {
            ...
            'note': self.note,
            'approved_by': self.env.user.employee_id.id if hasattr(self.env.user, 'employee_id') else False,
            'approved_date': fields.Datetime.now(),
        }
        
        existing_qc = qc_obj.search([('laundry_order_id', '=', self.order_id.id)], limit=1)
        if existing_qc:
